...
export function associationArn(parameters: AssociationArnParameters): AssociationArn {
  return `arn:${parameters.partition ?? 'aws'}:codeguru-reviewer:${parameters.region}:${parameters.account}:association:${parameters.resourceId}`
}
...
export function codereviewArn(parameters: CodereviewArnParameters): CodereviewArn {
  return `arn:${parameters.partition ?? 'aws'}:codeguru-reviewer:${parameters.region}:${parameters.account}:association:${parameters.resourceId}:codereview:${parameters.codeReviewId}`
}