...
> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:payment-cryptography:${this.region}:${this.account}:key/${this.keyId}` as const
  }
}
export type { KeyArn }
...
> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:payment-cryptography:${this.region}:${this.account}:alias/${this.alias}` as const
  }
}
