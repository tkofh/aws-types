...
export function groupArn(parameters: GroupArnParameters): GroupArn {
  return `arn:${parameters.partition ?? 'aws'}:xray:${parameters.region}:${parameters.account}:group/${parameters.groupName}/${parameters.id}`
}
...
export function samplingRuleArn(parameters: SamplingRuleArnParameters): SamplingRuleArn {
  return `arn:${parameters.partition ?? 'aws'}:xray:${parameters.region}:${parameters.account}:sampling-rule/${parameters.samplingRuleName}`
}