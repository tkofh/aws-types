...
> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:codeconnections:${this.region}:${this.account}:connection/${this.idConnection}` as const
  }
}
...
> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:codeconnections:${this.region}:${this.account}:host/${this.idHost}` as const
  }
}
export type { HostArn }
...
> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:codeconnections:${this.region}:${this.account}:repository-link/${this.idLinkRepository}` as const
  }
}
