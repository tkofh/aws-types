...
class BrokersArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'brokers', `arn:${string}:mq:${string}:${string}:broker:${string}:${string}`> {
...
export function brokersArn<Partition extends ArnPartition = 'aws'>(parameters: BrokersArnParameters<Partition>) {
  return new BrokersArn<Partition>(parameters)
}

export interface ConfigurationsArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly configurationId: string
}
class ConfigurationsArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'configurations', `arn:${string}:mq:${string}:${string}:configuration:${string}`> {
...
export function configurationsArn<Partition extends ArnPartition = 'aws'>(parameters: ConfigurationsArnParameters<Partition>) {
