...
class PipelineArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'pipeline', `arn:${string}:osis:${string}:${string}:pipeline/${string}`> {
...
export interface PipelineBlueprintArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly blueprintName: string
}
class PipelineBlueprintArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'pipeline-blueprint', `arn:${string}:osis:${string}:${string}:blueprint/${string}`> {
...
export function pipelineBlueprintArn<Partition extends ArnPartition = 'aws'>(parameters: PipelineBlueprintArnParameters<Partition>) {
