...
class PolicyArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'policy', `arn:${string}:fms:${string}:${string}:policy/${string}`> {
...
export interface ApplicationsListArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly id: string
}
class ApplicationsListArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'applications-list', `arn:${string}:fms:${string}:${string}:applications-list/${string}`> {
...
export function applicationsListArn<Partition extends ArnPartition = 'aws'>(parameters: ApplicationsListArnParameters<Partition>) {
  return new ApplicationsListArn<Partition>(parameters)
}

export interface ProtocolsListArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly id: string
}
class ProtocolsListArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'protocols-list', `arn:${string}:fms:${string}:${string}:protocols-list/${string}`> {
...
export function protocolsListArn<Partition extends ArnPartition = 'aws'>(parameters: ProtocolsListArnParameters<Partition>) {
...
class ResourceSetArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'resource-set', `arn:${string}:fms:${string}:${string}:resource-set/${string}`> {
