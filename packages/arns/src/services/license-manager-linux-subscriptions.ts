...
export function subscriptionProviderArn(parameters: SubscriptionProviderArnParameters): SubscriptionProviderArn {
  return `arn:${parameters.partition ?? 'aws'}:license-manager-linux-subscriptions:${parameters.region}:${parameters.account}:subscription-provider/${parameters.subscriptionProviderId}`
}