...
class ApplicationArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'Application', `arn:${string}:servicecatalog:${string}:${string}:/applications/${string}`> {
...
export interface AttributeGroupArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly attributeGroupId: string
}
class AttributeGroupArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'AttributeGroup', `arn:${string}:servicecatalog:${string}:${string}:/attribute-groups/${string}`> {
...
export function attributeGroupArn<Partition extends ArnPartition = 'aws'>(parameters: AttributeGroupArnParameters<Partition>) {
  return new AttributeGroupArn<Partition>(parameters)
}

export interface PortfolioArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly region: ArnRegion<Partition>
  readonly account: string
  readonly portfolioId: string
}
class PortfolioArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'Portfolio', `arn:${string}:catalog:${string}:${string}:portfolio/${string}`> {
...
export function portfolioArn<Partition extends ArnPartition = 'aws'>(parameters: PortfolioArnParameters<Partition>) {
...
class ProductArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'Product', `arn:${string}:catalog:${string}:${string}:product/${string}`> {
