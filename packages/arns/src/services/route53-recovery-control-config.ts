...
class ClusterArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'cluster', `arn:${string}:route53-recovery-control::${string}:cluster/${string}`> {
...
export interface ControlpanelArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly account: string
  readonly controlPanelId: string
}
class ControlpanelArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'controlpanel', `arn:${string}:route53-recovery-control::${string}:controlpanel/${string}`> {
...
export function controlpanelArn<Partition extends ArnPartition = 'aws'>(parameters: ControlpanelArnParameters<Partition>) {
...
export interface SafetyruleArnParameters<Partition extends ArnPartition = 'aws'> {
  readonly partition?: Partition | undefined
  readonly account: string
  readonly controlPanelId: string
  readonly safetyRuleId: string
}
class SafetyruleArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'safetyrule', `arn:${string}:route53-recovery-control::${string}:controlpanel/${string}/safetyrule/${string}`> {
...
export function safetyruleArn<Partition extends ArnPartition = 'aws'>(parameters: SafetyruleArnParameters<Partition>) {
