...
export function databaseArn(parameters: DatabaseArnParameters): DatabaseArn {
  return `arn:${parameters.partition ?? 'aws'}:timestream:${parameters.region}:${parameters.account}:database/${parameters.databaseName}`
}
...
export function tableArn(parameters: TableArnParameters): TableArn {
  return `arn:${parameters.partition ?? 'aws'}:timestream:${parameters.region}:${parameters.account}:database/${parameters.databaseName}/table/${parameters.tableName}`
}
...
export function scheduledQueryArn(parameters: ScheduledQueryArnParameters): ScheduledQueryArn {
  return `arn:${parameters.partition ?? 'aws'}:timestream:${parameters.region}:${parameters.account}:scheduled-query/${parameters.scheduledQueryName}`
}