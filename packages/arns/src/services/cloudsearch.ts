...
class DomainArn<Partition extends ArnPartition = 'aws'> extends InternalArn<'domain', `arn:${string}:cloudsearch:${string}:${string}:domain/${string}`> {
  ...
  [StringifyArnBrand]() {
    return `arn:${this.partition}:cloudsearch:${this.region}:${this.account}:domain/${this.domainName}` as const
  }
}
